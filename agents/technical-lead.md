...
Paste this block (with substitutions) at the top of the artifact this stage produces (`.context/developer-review-N.md`; N = `task.metadata.run_index`; resolver: metadata → newest glob `developer-review-*.md` → legacy `developer-review.md`).
...
- [ ] Your artifact (`.context/developer-review-N.md`) starts with `---
handoff:
` YAML frontmatter conforming to `skills/workflow/references/handoff-protocol.md`.
- [ ] Frontmatter includes all required fields for stage `DR` per the per-stage required-field matrix (see `analyzing-N.md#schemas`).
