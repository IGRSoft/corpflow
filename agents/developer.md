...
**Worktree Mode**: When `task.metadata.isolation === 'worktree'`, all operations use worktree path prefix. Use `EnterWorktree`/`ExitWorktree` tools to programmatically enter/leave worktree contexts. `EnterWorktree` accepts a `path` parameter (v2.1.105+) to target a specific worktree directory when multiple exist. Base-branch resolution is controlled by the `worktree.baseRef` setting (v2.1.133+): `head` (default — branch from local HEAD, matches v2.1.128 behavior) or `fresh` (branch from base ref, drops unpushed work). The plugin assumes `head` semantics; do not set `fresh` without coordinating with workflow-engineer. Build/test with `--package-path {workdir}`, git with `git -C {workdir}`. Stale worktrees are auto-cleaned (including those with untracked files, v2.1.98); fresh worktree per delegation (v2.1.119 — no more reuse of prior-session worktrees). Background-session dispatch (v2.1.142) now recognises pre-existing git worktrees (e.g., Conductor `.context` workspaces, externally-managed worktree shells) instead of refusing to spawn with a duplicate-creation error — `Edit` is no longer blocked when `EnterWorktree` would have collided. Background agents launched via `/bg` or `←←` preserve the active permission mode across retire/wake (v2.1.141) — a permissive `bypassPermissions` parent no longer reverts to `default` after the daemon hibernates. Sub-agents in isolated worktrees automatically get Read/Edit access to their own worktree (v2.1.101). For large repos, `worktree.sparsePaths` reduces checkout size. Stalled subagents now fail with a clear error after 10 minutes (v2.1.113) — surface and retry rather than waiting. Subagents resumed via `SendMessage` correctly restore their explicit spawn `cwd` (v2.1.118 fix). See `skills/milestone-workflow/SKILL.md`.
