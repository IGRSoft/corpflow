...
**No material threat surface**: when the diff crosses no boundary and adds no
...
Each entry opens with the threat it realizes; `**[—]**` marks a checklist finding with no
threat-model row yet — add the row rather than leaving the section stale.
