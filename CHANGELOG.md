...
- **`skills/shared/git-conventions.md` gains § Branch Naming** — the canonical single source of truth for branch grammar (`<type>/<slug>`, no ticket), type vocabulary (12 tokens: `feat`, `feature`, `fix`, `refactor`, `perf`, `docs`, `chore`, `test`, `ci`, `build`, `style`, `revert`), guard ladder (5 arms: conventional, upstream, integration branch, target exists, detached/not-a-repo), and the once-only rule. Eight other files now reference this section instead of restating rules.
- **Branch type vocabulary extended**: both `feat` (short form) and `feature` (long form) are now accepted as conventional prefixes. The new PL entry point emits the long form (`feature/<slug>`); existing `feat/*` branches remain valid and conventional. The already-conventional check accepts both forms, so existing short-form branches are never churned.
...
- Suite: 340 bats assertions across 41 files, 48 Swift Testing cases, 37 Python unittest cases, 175 benchmark-harness tests — **all green, 0 failures, exit 0**.
