...
3. End with a single ready-to-paste `worktask:` trigger line (the only trigger; PL0 dynamic sizing
   handles small tasks by dropping stages). Exception: for XL-sized work, emit no trigger — instead
   list 2–3 sub-tasks to split into per `references/handoff.md`.
