...
- Plugin version: **4.0.23** — CI pipeline wired from 0 to green (`.github/workflows/test.yml` added with shell-bash pipefail gate so red suites report red not green, fetch-depth per-job regression-tested and mutation-verified, four stale "not wired to CI" claims cleared, `disable-model-invocation` applied to two pipeline-only skills, 120 keywords consolidated to 15 moving release-note tokens to CHANGELOG.md, gate labels disambiguated)
