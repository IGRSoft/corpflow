...
- Plugin version: **3.38.0** (compatible dev-plugin registry: new `skills/shared/compatible-plugins.md` holds plugin-level metadata only — registry, functional-role agent roster, core-parity command set, handoff defaults — with agent routing left canonical in `platform-detection.md`; new `cross-plugin-handoff/references/plugin-onboarding.md` carries the compatibility contract + 14-row add/replace touchpoint checklist. `ai-engineer` wired in from zero references (`ai` platform key, markers, six Task grants). Gap fixes: `publish-pl-issue.sh` prefix regex named only apple-developer at all four sites so five plugins' agent ids leaked into published issues; pm-milestone routed android/web to `igrsoft:developer`; AR/SR/QA agents could reach only `apple-developer:*`; frontend/backend/ai had no `plugin-protocols.md` stage tables. MINOR; min CC unchanged 2.1.220. Prior 3.37.1 = model-name sweep.)
