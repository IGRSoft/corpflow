...
- Plugin version: **4.0.26** — command-surface reorganization: 38 → 28 commands and 26 → 23 registered skills. Seven commands removed with no alias (`business-report`, `pm-prioritize`, `pm-risk`, `test-report`, `cost-report`, `agent-report`, `worktask-status`, `context-status`), five renamed (`pm-*` prefix dropped; `dev-code-review` → `tech-code-review`), the three Apple-only `appstore-*` commands moved to apple-developer behind a new `/appstore` front door, `senior-developer-review` folded into `estimation-methodology/references/estimate-review.md`, and cost observability retired entirely (its JSONL trail was never written by any shipped hook)
