...
- Plugin version: **3.25.0** (Restored PL plan-approval gate + single `worktask` trigger — `plan_gate` default `checkpoint`, `--auto-plan` bypass, micro/quick/fworktask triggers collapsed to single worktask. ~18 files reconciled, docs/metadata only)
