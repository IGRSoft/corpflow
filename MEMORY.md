...
- Plugin version: **4.0.23** — CI pipeline wired from 0 to green (`.github/workflows/test.yml` added, `shell: bash` ensures pipefail gates the suite step so red suites report red not green, fetch-depth per-job pinned in contract + assertion, four stale "not wired" claims cleared, `disable-model-invocation` applied to two pipeline-only skills, 120 keywords consolidated to 15, SKIPPED_PHASES false-positive eliminated)
- Plugin version: **4.0.22** — superpowers-patterns adoption: trigger-first `description:` grammar across 42 assets (commands exempt), rationalization tables + Red Flags added to 6 agents, `desc-lint.sh` extended from 1 check to 6 (G1–G6) with full skill-tree discovery, authoring doctrine consolidated in `agents/prompt-engineer.md`, PL stage made proactive (announced sizing, assumption-tagged questions, one-way re-sizing ratchet)
...
- 2026-08-22: v4.0.23 — CI pipeline established from scratch (repo had no CI at all): `.github/workflows/test.yml` added with shell-bash pipefail gate, fetch-depth per-job assertion + contract documentation in architecture-0.md, new `tests/shell/meta/ci-workflow.bats` (16 assertions, including one new regression test for fetch-depth per job independently mutation-verified non-vacuous), matrix-row R28 for CI workflow tests, four stale "not wired to CI" claims cleared across three docs, disabled model invocation on two pipeline-only skills (`preview-ensurer`, `csv-export-templates`), manifest keywords reduced 120→15 per marketplace spec, marketplace.json mirrored keywords + version into plugin.json, SKIPPED_PHASES misleading header removed (designed conditional, not suppressed defect).
