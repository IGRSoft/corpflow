...
- Plugin version: **4.0.25** — plan-approval carrier `PL0.metadata.approved` gained its missing writer (three plan-gate arms in `commands/worktask.md` stamp user/auto/user with the stamp ordered before the audit row, Step A.4 documented as deliberately non-stamping, megatask per-issue seed stamps `auto`, three reader sites carrier-qualified, schema `description` added, new `approval-gate.bats` with non-vacuous cross-file extraction)
