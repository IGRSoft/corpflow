...
> **DV Hotfix Tip (v2.1.98+)**: Use the Monitor tool to stream build output during hotfix implementation. Combine `run_in_background` Bash with Monitor for real-time error detection instead of polling. See `${CLAUDE_SKILL_DIR}/../agent-coordination/SKILL.md §Monitor Tool`.
