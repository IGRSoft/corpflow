...
**When the user's message starts with ANY of these prefixes, Claude MUST immediately invoke the workflow command BEFORE doing anything else:**

| Prefix | Required Action |
|--------|-----------------|
| `workflow:` | Invoke `/company-workflow:workflow "[task]"` |
| `fworkflow:` | Invoke `/company-workflow:workflow "[task]" --fast` |
| `quick:` | Invoke `/company-workflow:workflow "[task]" --quick` |
...
3. If prefix is NOT "micro:", invoke SlashCommand tool with workflow
...
3. Use SlashCommand tool: `/company-workflow:workflow "/apple-developer:code-legacy-modernize migrate @StateObject to @Environment"`
4. The workflow sets up context, then the workflow stages handle execution
...
3. Use SlashCommand tool: `/company-workflow:workflow "Fix typo in login button" --fast`
...
3. Use SlashCommand tool: `/company-workflow:workflow "Add dark mode toggle" --quick`
