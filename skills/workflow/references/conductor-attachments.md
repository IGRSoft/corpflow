...
(F Stage) and validated by `skills/shared/stage-contracts.md` (FN row).
...
In `agents/project-manager.md § F Stage`, immediately **before** invoking
`gh pr create`:
...
overwritten on every FN run (idempotent).
...
| Conventional-commit type | Derived from `.context/planning.md § Goal` (feat/fix/refactor/perf/docs/chore/test/ci/build/style); falls back to `feat` |
...
- `agents/project-manager.md § F Stage` — the writer
