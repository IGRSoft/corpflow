...
case "$CMD" in
  ...
  self-test)
    ...
    SELFTEST_LIB_PATH="$(dirname "${BASH_SOURCE[0]}")/dv-tree-preflight-selftest.sh"
    if [ -r "$SELFTEST_LIB_PATH" ]; then
      # shellcheck source=dv-tree-preflight-selftest.sh
      # shellcheck disable=SC1090
      . "$SELFTEST_LIB_PATH"
    else
      printf >&2 'dv-tree-preflight: self-test harness unreachable at %s — plugin install broken\n' \
        "$SELFTEST_LIB_PATH"
      exit 2
    fi
    run_self_test
