...
10. **Branch naming** (first stage only, after the state.json seed and before `TaskCreate` PL0): run `bash skills/worktask/scripts/branch-name.sh --goal "<task description>"` — the ONLY point in the pipeline a worktask branch is ever renamed (once-only rule, `skills/shared/git-conventions.md § Branch Naming`). Every outcome exits 0 and the step self-disables under `/megatask`/`--emergency` routing; capture its final `branch=<name>` stdout line and stamp `facts.branch` on the ledger (the script itself never writes state.json — see `references/handoff-protocol.md § branch`).
